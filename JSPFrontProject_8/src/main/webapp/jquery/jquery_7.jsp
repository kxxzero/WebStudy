 ...
 */
 	...
 	$(function(){
		...
		// 이벤트 처리
		$('#keyword').keyup(function(){ // 키 입력 시
			let k=$('#keyword').val();
			$('#user-table > tbody > tr').hide() // 객체 숨김
			let temp=$('#user-table > tbody > tr > td:nth-child(5n+3):contains("'+k+'")') // 영화명만 탐색할 수 있는 수열 생성
			$(temp).parent().show()
		})
	})
</script>
